...
class MyDataset4Train(DGLDataset):
    ...
    def process(self):
        ...
        self.graph_patch = []
        self.graph_clean = []
        with open(Config.graph_map_path, "r", encoding="utf-8", errors="ignore") as r:
    ...
    def save(self):
        ...
        graph_path = os.path.join(os.path.join(Config.processed_msg_path, "bug_graphs.bin"))
        save_graphs(graph_path, self.graph_bug)

        graph_path = os.path.join(os.path.join(Config.processed_msg_path, "patch_graphs.bin"))
        save_graphs(graph_path, self.graph_patch)

        graph_path = os.path.join(os.path.join(Config.processed_msg_path, "clean_graphs.bin"))
        save_graphs(graph_path, self.graph_clean)

    def load(self):
        self.dataset = []
        bug_graphs, _ = load_graphs(os.path.join(Config.processed_msg_path, 'bug_graphs.bin'))
        patch_graphs, _ = load_graphs(os.path.join(Config.processed_msg_path, 'patch_graphs.bin'))
        clean_graphs, _ = load_graphs(os.path.join(Config.processed_msg_path, 'clean_graphs.bin'))
    ...
    def has_cache(self):
        bug_data_path = os.path.join(Config.processed_msg_path, "bug_graphs.bin")
        patch_data_path = os.path.join(Config.processed_msg_path, "patch_graphs.bin")
        clean_data_path = os.path.join(Config.processed_msg_path, "clean_graphs.bin")
...
class MyDataset4Test(DGLDataset):
    ...
    def save(self):
        ...
        graph_path = os.path.join(os.path.join(Config.processed_msg_path, "test.bin"))
        self.label = torch.tensor(self.label, dtype=torch.float32)
        save_graphs(graph_path, self.dataset, {"labels": self.label})

    def load(self):
        self.dataset = []
        self.dataset, self.label = load_graphs(os.path.join(Config.processed_msg_path, 'test.bin'))
        self.label = self.label["labels"]

        self.data_len = len(self.dataset)

    def has_cache(self):
        test_data_path = os.path.join(Config.processed_msg_path, "test.bin")
