...
class Evaluation:
    ...
    def evaluate_4_nn(self):
        precision, recall, f1, balanced_acc, cls_report = self.__get_eval_indicies()
    ...
    def __get_eval_indicies(self):
        ...
        return precision, recall, f1, balanced_acc, cls_report
    ...
    def plot_roc(self, X, y):
        ...
        with tqdm(total=10, desc='(T)') as pbar:
            ...
            for i, (train, test) in enumerate(cv.split(X, y)):
                ...
                p = precision_score(y[test], y_pred)
                r = recall_score(y[test], y_pred)
                f = f1_score(y[test], y_pred)
                ...
                f1s.append(f)
                accus.append(a)
                classification_result.append([p, r, f, a])
        ...
        f_a = sum(f1s) / len(f1s)
        a_a = sum(accus) / len(accus)
        print("precision_avg: {}".format(p_a))
        print("recall_avg: {}".format(r_a))
        print("f1_avg: {}".format(f_a))
        print("accuracy_avg: {}".format(a_a))

        classification_result.append([p_a, r_a, f_a, a_a])

        print("precision_max: {}".format(max(precisions)))
        print("recall_max: {}".format(max(recalls)))
        print("f1_max: {}".format(max(f1s)))
        print("accuracy_max: {}".format(max(accus)))
